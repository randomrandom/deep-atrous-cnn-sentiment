...
class BasePreprocessor(object):
    ...
    def __init__(self, path, filename, separator, vocabulary_size, max_data_length, test_split, pad_token=_PAD_TOKEN,
    ...
    def save_preprocessed_file(self):
        ...
        self.train_size = (int)(data_size * (1 - self.test_split))
        self.test_size = data_size - self.train_size

        self.new_data.iloc[:self.train_size, :].to_csv(self.path + self.CLEAN_PREFIX + self.filename, sep=self.separator,
                                                  index=False)
        self.new_data.iloc[self.train_size:, :].to_csv(self.path + self.TEST_PREFIX + self.filename, sep=self.separator,
                                                  index=False)
